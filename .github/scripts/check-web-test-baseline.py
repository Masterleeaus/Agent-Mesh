...
def main() -> int:
    ...
    if args.command_status != 0 and not current:
        print(
            "Web tests failed but no named Vitest failures were parsed; "
    ...
    print(
        f"Web test baseline: current={current_total}, "
    ...
    if regressions:
        print("New/increased web test failures:", file=sys.stderr)
        for key, count, limit in regressions:
            print(f"  - {key}: {count} > allowed {limit}", file=sys.stderr)
        return 1

    if args.command_status == 0:
        print("Web tests are fully clean; baseline can be deleted.")
    else:
        print("No web test regression beyond the recorded Merge84 baseline.")
    return 0

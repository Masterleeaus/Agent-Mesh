...
SID=re.compile(r"^TZ-(?:G00|ROADMAP-\d+)-SG-\d+$"); SHA=re.compile(r"^[0-9a-f]{40}$",re.I)
...
def validate(p):
 ...
 sid=str(p.get("subgoal_id") or ""); branch=str(p.get("claim_branch") or "")
 ...
 return {"subgoal_id":sid,"claim_branch":branch,"expected_head_sha":p["expected_head_sha"],"operation":op,"parameters":params}
def self_test():
 good={"schema":"titan-agent-mesh-execution-request/v1","subgoal_id":"TZ-ROADMAP-55-SG-01","claim_branch":"agent/TZ-ROADMAP-55-SG-01","expected_head_sha":"a"*40,"operation":"inventory.repository","parameters":{}}

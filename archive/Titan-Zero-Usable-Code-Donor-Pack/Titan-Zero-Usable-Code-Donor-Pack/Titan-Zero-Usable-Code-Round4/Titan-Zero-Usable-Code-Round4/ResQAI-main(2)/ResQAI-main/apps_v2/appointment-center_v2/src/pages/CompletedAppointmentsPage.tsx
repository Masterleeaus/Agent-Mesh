...
export function CompletedAppointmentsPage() {
  const [search, setSearch] = useState('');
  const [page, setPage] = useState(1);
  const { appointments, total, loading, error, refetch } = useAppointments({ status: [AppointmentStatus.Completed], pageSize: 50 });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div>
        <h1 style={{ margin: '0 0 4px', fontSize: 22, fontWeight: 700, color: '#e2e8f0' }}>Completed Appointments</h1>
        <p style={{ margin: 0, fontSize: 13, color: '#64748b' }}>{total} completed appointment{total !== 1 ? 's' : ''}</p>
      </div>

      <Card variant="elevated" padding="md">
        <div style={{ marginBottom: 16 }}>
          <SearchBar value={search} onChange={setSearch} placeholder="Search completed appointments..." />
        </div>

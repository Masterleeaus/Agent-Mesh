...
- **Email › Ingest Portal Email** — optional **Source Email** JSON (`sourceEmail`) on the existing `ingestEmail` action. When set, it is sent unchanged on `POST /ingest/email/portal`. When omitted or empty, the request matches 0.11.3. The node output includes the echoed `sourceEmail` from the Intelligence API (`null` when omitted). Tenant `company_id` and `api_key` still come only from the VH3 AI credential.

## [0.11.3] — 2026-08-25

### Added

- **Case › Delete Case** — archives a case through the existing transition path (`POST /cases/{id}/transition` with `status: archived`). The record, linked items, and activity stay. Does not permanently delete or restore. A second delete on an already-archived case fails the same way Transition Case to archived would.
...
- **Team › List Members** — docs now match the tenant-key members proxy: each row includes `user_name` and `user_email`. Notification workflows can use List Members output without joining List Users.
...
- **Job Feed filter fields** — boolean descriptions now start with “Whether” (n8n community linter). Sort By options ordered alphabetically: Actual End, Actual Start, Created At, End Delta (Mins), Start Delta (Mins).

## [0.10.11] — 2026-08-05

### Added

- **Job Feed › List / Account / Aggregate** — optional **Finished Late** / **Started Late** boolean Additional Fields (`finished_late` / `started_late` on feed; `finishedLate` / `startedLate` in aggregate `filters`). Prefer these set to On for late jobs instead of Early set to Off. When both Early and Late are set for the same axis, only Late is sent.

  ...
  const row = await portableQueryOne(
    `SELECT p.*, c.name AS client_name,
            ...
            (SELECT COUNT(DISTINCT v.id)
               FROM visits v JOIN jobs jv ON jv.id = v.job_id AND jv.account_id = v.account_id
            ...
            (SELECT MIN(v2.scheduled_start)
              ...
              WHERE j2.property_id = p.id AND v2.account_id = p.account_id
                AND v2.status = 'scheduled' AND v2.scheduled_start > NOW()) AS next_visit_at,
            (SELECT COUNT(*) FROM jobs aj
              WHERE aj.property_id = p.id AND aj.account_id = p.account_id
                AND aj.status IN ('scheduled', 'in_progress')) AS active_jobs,
            (SELECT COUNT(*) FROM estimates pe
              WHERE pe.property_id = p.id AND pe.account_id = p.account_id
                AND pe.status IN ('draft', 'sent')) AS pending_estimates,
            (SELECT COALESCE(SUM(oi.total_cents), 0) FROM invoices oi
              WHERE oi.property_id = p.id AND oi.account_id = p.account_id
                AND oi.status IN ('sent', 'partial', 'overdue')) AS outstanding_cents,
            (SELECT COUNT(*) FROM property_issues pi2
              WHERE pi2.property_id = p.id AND pi2.account_id = p.account_id
                AND pi2.status IN ('open', 'monitoring')) AS open_issues_count
     FROM properties p
...
export const PATCH = withRole(["owner", "admin"], async (request: NextRequest, session: AuthSession) => {
  ...
  try {
    const updated = await withPortableTransaction(async (client) => {
      const before = await client.query<Record<string, unknown>>(`SELECT * FROM properties WHERE id = $1 AND account_id = $2`, [id, session.accountId]);
      if (before.rows.length === 0) {
        const error = new Error("PROPERTY_NOT_FOUND");
        (error as Error & { code?: string }).code = "PROPERTY_NOT_FOUND";
        throw error;
      }
      ...
      if (!row) throw new Error("Property update did not return persisted row");
    ...
    return NextResponse.json({ error: { code: "INTERNAL_ERROR", message: "Failed to update property", traceId: session.traceId } }, { status: 500 });
  }
});

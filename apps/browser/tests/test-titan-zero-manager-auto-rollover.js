...
for(const f of ['manager-lifecycle','manager-self-claim','manager-auto-rollover'])vm.runInContext(fs.readFileSync(path.join(root,'src/titan-zero',f+'.js'),'utf8'),s,{filename:f});
const R=s.TitanZeroManagerAutoRollover;
assert.throws(()=>R.plan('Agent 6',{lane:'manager_core'},{subgoal_id:'SG722',state:'DONE',source:'legacy-projection'},[],{}),/GitHub-backed/);assert.throws(()=>R.plan('Agent 6',{lane:'manager_core'},{subgoal_id:'SG722',state:'PROMOTED',source:'legacy-projection'},[],{}),/GitHub-backed/);
...
let p=R.plan('Agent 6',agent,{subgoal_id:'SG722',state:'MERGED',source:'github'},issues,{claimBranches:[],openPRs:[],dependencyState:{byIssue:{SG723:{eligible:true},SG724:{eligible:true}}}});
...
p=R.plan('Agent 6',agent,{subgoal_id:'SG722',state:'MERGED',source:'github'},issues,{claimBranches:['agent/SG723','agent/SG724']});
...
p=R.plan('Agent 6',agent,{subgoal_id:'SG722',state:'MERGED',source:'github'},issues,{openPRs:[{state:'OPEN',headBranch:'agent/SG723'}],claimBranches:[]});

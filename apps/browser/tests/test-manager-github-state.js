...
x=G.derive({issue:{number:722,subgoal_id:'SG722',state:'OPEN'},mainSha:'c'.repeat(40),baseSha:main,headSha:head,branch:'agent/SG722',claimBranchExists:true,compare:{behind_by:2,ahead_by:3}});
assert.strictEqual(x.git.behindMain,true);assert.strictEqual(x.git.behindBy,2);assert.strictEqual(x.git.aheadBy,3);
assert.throws(()=>G.derive({issue:{subgoal_id:'SG722'},mainSha:'not-a-sha'}),/invalid GitHub main SHA/);

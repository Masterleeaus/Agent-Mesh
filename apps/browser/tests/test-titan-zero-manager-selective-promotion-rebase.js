...
assert.equal(W.authority.repositoryBaseline,'git-main-sha');assert.equal(W.authority.generation,'projection-metadata-only');assert(W.LIFECYCLE.includes('PR_OPEN'));assert(!W.LIFECYCLE.includes('PROMOTED'));assert(!W.LIFECYCLE.includes('CONVERGENCE_PENDING'));

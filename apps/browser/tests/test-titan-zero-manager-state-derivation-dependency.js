...
const noGithub=S.derive({packets:[{packet_id:'LEGACY',status:'AVAILABLE'}],claims:[],deltas:[]});assert.strictEqual(noGithub.source,'unavailable');assert.strictEqual(noGithub.status,'GITHUB_STATE_REQUIRED');assert.strictEqual(noGithub.failClosed,true);assert.strictEqual(noGithub.authority.mayUnlockDependency,false);
const legacy=S.derive({compatibilityMode:true,packets:[{packet_id:'LEGACY',status:'AVAILABLE'}],claims:[],agents:[],deltas:[],handoffs:[],verification:[]});assert.strictEqual(legacy.source,'legacy-projection');assert.strictEqual(legacy.queue.nextGlobal,null);assert.deepStrictEqual(legacy.queue.eligible,[]);assert.strictEqual(legacy.authority.mayClaim,false);assert.strictEqual(legacy.authority.mayComplete,false);assert.strictEqual(legacy.authority.mayRequestMerge,false);assert.strictEqual(legacy.diagnostics.dependency.byPacket && Object.keys(legacy.diagnostics.dependency.byPacket).length,0);
...
const unavailable=S.derive({packets:[]});assert.strictEqual(unavailable.source,'unavailable');assert.strictEqual(unavailable.failClosed,true);assert(unavailable.drift.some(x=>x.code==='github-state-required'));
...
const unavailable=S.derive({packets:[{packet_id:'LEGACY',status:'AVAILABLE'}],claims:[{packet:'LEGACY',status:'ACTIVE'}]});assert.strictEqual(unavailable.source,'unavailable');assert.strictEqual(unavailable.status,'GITHUB_STATE_REQUIRED');assert.strictEqual(unavailable.failClosed,true);assert.deepStrictEqual(unavailable.queue.eligible,[]);assert.strictEqual(unavailable.authority.mayClaim,false);assert.strictEqual(unavailable.authority.mayUnlockDependency,false);
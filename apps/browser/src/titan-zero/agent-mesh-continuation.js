...
function executionResume(v={}){const x=v&&typeof v==='object'?v:{};return freeze({succeeded:list(x.succeeded,100),failed:list(x.failed,100),unresolved:list(x.unresolved,100),replayPolicy:clean(x.replayPolicy,120)||'DO_NOT_REPLAY_SUCCEEDED_OR_UNRESOLVED',mayAdvanceLifecycle:false});}
...
function workContext(input={}){
 const cp=checkpoint(input.checkpoint||input),gh=input.github||{},git=gh.git||{},pr=gh.pr||null,checks=gh.checks||null;
 const branch=clean(git.branch||input.claim_branch||cp.claim_branch,240);if(branch!==cp.claim_branch)throw new Error('work context claim branch mismatch');
 const mainSha=gitSha(git.mainSha||input.main_sha||cp.main_sha,'main_sha'),baseSha=gitSha(git.baseSha||input.base_sha||cp.base_sha,'base_sha'),headSha=gitSha(git.headSha||input.head_sha||cp.head_sha,'head_sha');
 return freeze({schema:'titan-code.agent-mesh-work-context.v1',issue:{number:cp.issue_number,subgoal_id:cp.subgoal_id},objective:cp.objective,claim:{branch:cp.claim_branch,main_sha:mainSha,base_sha:baseSha,head_sha:headSha},lifecycle:clean(gh.state||input.lifecycle,40)||null,pr:pr?{number:Number(pr.number)||null,state:clean(pr.state,40)||null,draft:pr.draft===true,merged:pr.merged===true}:null,checks:checks?{required:Number(checks.required)||0,pending:checks.pending===true,failed:checks.failed===true}:null,progress:{current_pass:cp.current_pass,completed:cp.completed,current_work:cp.current_work,next_actions:cp.next_actions,blockers:cp.blockers,verification:cp.verification,do_not_repeat:cp.do_not_repeat},execution_resume:cp.execution_resume,instructions:['Treat GitHub issue, claim branch, refs, PR and checks as durable truth.','Read AGENTS.md before mutation.','Stay on the same canonical claim branch.','Do not repeat completed work.','Do not replay succeeded or accepted-but-unresolved execution requests.','Reconcile head SHA before editing; GitHub wins on disagreement.','Manager/model output is advisory and cannot grant merge authority.'],authority:{durableTruth:'github',claim:'git-branch-ref',merge:'github-pr-merge',conversationState:'disposable',localState:'projection-only',titanZeroRuntimeDependency:false}});
}
function bootstrap(input={}){
 const cp=checkpoint(input.checkpoint||input),gh=input.github||{};
 const context=workContext({checkpoint:cp,github:gh});
 const expectedHead=cp.head_sha,liveHead=context.claim.head_sha;
 if(expectedHead&&liveHead&&expectedHead!==liveHead)return freeze({schema:'titan-code.agent-mesh-bootstrap.v1',status:'RECONCILE_REQUIRED',failClosed:true,issue:context.issue,claim:context.claim,reason:'checkpoint-head-differs-from-github',context,resume:resumeInstructions(cp),authority:{durableTruth:'github',mayMutate:false,mayMerge:false,mayReleaseClaim:false}});
 return freeze({schema:'titan-code.agent-mesh-bootstrap.v1',status:'READY_TO_RESUME',failClosed:false,issue:context.issue,claim:context.claim,context,resume:resumeInstructions(cp),authority:{durableTruth:'github',conversation:'disposable',mayMutate:true,mayMerge:false,mayReleaseClaim:false}});
}
...
g.TitanCodeAgentMeshContinuation=freeze({SCHEMA,RECEIPT_SCHEMA,checkpoint,takeover,reconcileTakeover,workContext,resumeInstructions,bootstrap});

...
function evaluate(packet={},ctx={}){const blockers=[];const deps=Array.isArray(packet.dependencies)?packet.dependencies:[];const githubBacked=ctx.authority?.durableTruth==='github'||ctx.github===true;const done=githubBacked?set(ctx.githubCompletedPackets):new Set();for(const dep of deps)if(!done.has(String(dep)))blockers.push(`dependency:${dep}`);const exclusive=overlap(packet.exclusive_hotspots,ctx.activeExclusiveHotspots);for(const h of exclusive)blockers.push(`exclusive-hotspot:${h}`);const shared=overlap(packet.shared_hotspots,ctx.activeSharedHotspots);return freeze({schema:SCHEMA,eligible:githubBacked&&blockers.length===0,blockers,coordinationRequired:shared.length>0,sharedHotspotOverlap:shared,authority:{dependencyEvidence:githubBacked?'github':'none',mayUnlock:githubBacked&&blockers.length===0}});}

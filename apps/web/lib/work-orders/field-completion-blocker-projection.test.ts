...
describe("field completion blocker projections",()=>{
 ...
 it("failed inspections project a blocker while passed inspections resolve it",async()=>{
  const failed=client();await syncInspectionCompletionBlockers(failed,"a1",{company_id:"c1",work_order_id:"wo1",inspection_id:"i1",result:"failed"});
  expect(failed.query).toHaveBeenCalledTimes(2);expect(failed.query.mock.calls[1][1][5]).toBe("FAILED_INSPECTION_UNRESOLVED");
  const passed=client();await syncInspectionCompletionBlockers(passed,"a1",{company_id:"c1",work_order_id:"wo1",inspection_id:"i1",result:"passed"});
  expect(passed.query).toHaveBeenCalledTimes(1);
 });

...
import {
  buildTitanSalesPlan,
  type TitanSalesPlanInput,
} from "@ai-fsm/titan-platform/workforce-native";

export type SalesNativeRequest = Omit<TitanSalesPlanInput, "companyId" | "actorId"> & {
  dryRun?: boolean;
};

const ALLOWED_ROLES = new Set(["owner", "admin"]);

function assertSalesRole(session: AuthSession): void {
...
function materializePath(path: string, entityId: string | null): string {
  ...
  if (!entityId) throw new Error("sales-request-id-required");
...
export function buildNativeSalesPlan(session: AuthSession, input: SalesNativeRequest) {
  assertSalesRole(session);
  return buildTitanSalesPlan({
    ...input,
...
export async function executeNativeSalesAction(
  request: Request,
  session: AuthSession,
  input: SalesNativeRequest,
) {
  const plan = buildNativeSalesPlan(session, input);
  ...
  headers.set("x-titan-workforce-agent", "sales");

...
export function RoleDetails({ role, command, demo = true }: { role: TitanRole; command: React.ReactNode; demo?: boolean }) {
  if (!demo && role !== "zero") return <section className="role-detail-layout"><div className="field-empty-state"><ShieldCheck/><strong>Authenticated operational details</strong><span>Prototype job, customer, access-code, route and payment records are hidden. Open the existing operational workspace for live company data.</span></div></section>;

...
export function RoleInbox({ role, command, demo = true }: { role: TitanRole; command: React.ReactNode; demo?: boolean }) { if (!demo && role !== "zero") return command; if (role === "go") return <GoInbox/>; if (role === "hub") return <HubInbox/>; return command; }
...
export function RoleMore({ role, command, demo = true }: { role: TitanRole; command: React.ReactNode; demo?: boolean }) {
  if (role === "zero" || !demo) return command;

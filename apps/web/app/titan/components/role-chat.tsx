...
export function RoleChat({ role, onOpenDetails, projection: suppliedProjection, demo = suppliedProjection === undefined }: { role: TitanRole; onOpenDetails: () => void; projection?: ReturnType<typeof getDemoSurfaceProjection>; demo?: boolean }) {
  ...
  const [intent, setIntent] = useState(demo ? (role === "zero" ? "needs me" : "next booking") : "");
  ...
  const response = useMemo(() => demo ? answerFor(role, intent) : null, [demo, intent, role]);
  ...
  return <div className={`role-chat role-${role}`} style={{ "--role-accent": profile.accent } as React.CSSProperties}>
    <section className="chat-stage">
      ...
      <footer className="chat-footer"><span><Check />{demo ? "Demo workspace" : "Authenticated workspace"}</span><span>{demo ? "PWA delivery prototype" : "Network-backed workspace · local conversation continuity only"}</span></footer>

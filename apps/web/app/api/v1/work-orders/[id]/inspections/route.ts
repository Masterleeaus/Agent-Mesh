...
import { buildTitanFieldInspection } from "@titan-zero/titan-platform/business-ops";
export const dynamic="force-dynamic";
const schema=z.object({inspection_id:z.string().min(1),permit_id:z.string().min(1),inspection_date:z.string(),result:z.enum(["scheduled","passed","failed","cancelled"])});
export const POST=withRole(["owner","admin"],async(request:NextRequest,session:AuthSession)=>{
 ...
 try{await withPortableTransaction(async(client)=>{
  const permit=await client.query<{id:string}>(`SELECT id FROM field_permits WHERE id=$1 AND company_id=$2 AND account_id=$3 AND work_order_id=$4 FOR UPDATE`,[parsed.data.permit_id,canonicalCompanyIdFromSession(session.accountId),session.accountId,id]);if(!permit.rows[0])throw new Error("PERMIT_NOT_FOUND");
  ...
  const inspection=buildTitanFieldInspection({...parsed.data,company_id:canonicalCompanyIdFromSession(session.accountId),provenance});
  await recordGovernedInspectionState(client as DbClient,{accountId:session.accountId,company_id:inspection.company_id,actorId:session.userId,traceId:session.traceId,role:session.role},{...inspection,work_order_id:id});

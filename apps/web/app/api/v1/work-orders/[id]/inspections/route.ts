...
export const POST=withRole(["owner","admin"],async(request:NextRequest,session:AuthSession)=>{
 ...
 try{await withPortableTransaction(async(client)=>{
  ...
  await recordGovernedInspectionState(client,{accountId:session.accountId,company_id:inspection.company_id,actorId:session.userId,traceId:session.traceId,role:session.role},{...inspection,work_order_id:id});

...
export const POST = withRole(["owner", "admin"], async (request: NextRequest, session: AuthSession) => {
  ...
  try {
    const result = await withPortableTransaction(async (client) => {
      const wo = await client.query<{ id: string; client_id: string; property_id: string | null; job_id: string | null }>(
      ...
      const calc = await calculateTravelForAccount(client, session.accountId, {
        ...
        custom_total_cents: data.custom_total_cents, trip_count: data.trip_count,
        trip_direction: data.trip_direction, manual_one_way_miles: data.manual_one_way_miles,
        manual_one_way_minutes: data.manual_one_way_minutes,
      });
      const snapshot = await insertTravelSnapshot(client, {
        account_id: session.accountId, origin_address: calc.origin_address,
        destination_address: calc.destination_address, result: calc.calculation,
        calculation_source: calc.calculation_source, trip_calculation_method: calc.trip_calculation_method,
        mileage_rate_id: calc.mileage_rate_id,
        manually_overridden: data.charge_mode === "waive" || data.charge_mode === "custom" || data.manual_one_way_miles != null,
        override_reason: data.override_reason ?? null, work_order_id: workOrderId,
        job_id: row.job_id, kind: "estimate", created_by: session.userId,
      });
      await client.query(`UPDATE work_orders SET travel_snapshot_id = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 AND account_id = $3`, [snapshot.id, workOrderId, session.accountId]);
    ...
    return NextResponse.json({ error: { code: "INTERNAL_ERROR", message: "Failed to attach travel", traceId: session.traceId } }, { status: 500 });
  }
});

export const GET = withRole(["owner", "admin", "tech"], async (request: NextRequest, session: AuthSession) => {
  const workOrderId = workOrderIdFromPath(request.nextUrl.pathname);
  const wo = await portableQuery<{ travel_snapshot_id: string | null }>(`SELECT travel_snapshot_id FROM work_orders WHERE id = $1 AND account_id = $2`, [workOrderId, session.accountId]);
  if (!wo.length) return NextResponse.json({ error: { code: "NOT_FOUND", message: "Work order not found", traceId: session.traceId } }, { status: 404 });
  if (!wo[0].travel_snapshot_id) return NextResponse.json({ data: null });
  const snap = await portableQuery(`SELECT * FROM travel_calculation_snapshots WHERE id = $1 AND account_id = $2`, [wo[0].travel_snapshot_id, session.accountId]);
  return NextResponse.json({ data: snap[0] ?? null });
});
